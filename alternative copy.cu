...
void calc_sha_256(uint8_t hash[SIZE_OF_SHA_256_HASH], const void *input, size_t len, uint8_t *inthash, int * correctwordnumber)
{
	...
	hash = hash + (sizeof(uint8_t) * SIZE_OF_SHA_256_HASH) * index;
	const void *tmpstr = input + (sizeof(char) * len + 1) * index;
	// orginal
	struct Sha_256 sha_256;
	Sha_256 *s = &sha_256;
	sha_256_init(s, hash);
	sha_256_write(s, tmpstr, len);
	(void)sha_256_close(s);
	// end orginal
	int tmp = 0;
        for (int i = 0; i < 32; i++)
	{
...
int main(void)
{
    ...
    // Words per Run
    int wordsperrun = (threadsPerBlock * numberOfBlocks);
    printf("Words per run: %d\n", wordsperrun);

    // Size of Buffer for all passwords
    char *h_buff;
    char *d_buff;
	int buff_size = (sizeof(char) * (wordsize + 1) * wordsperrun);
	h_buff=(char *)malloc(buff_size);
	cudaMalloc((void **)&d_buff, buff_size);
    ...
    uint8_t *h_buffhash;
    uint8_t *d_buffhash;
	int buffhash_size = (sizeof(uint8_t) * 32 * wordsperrun);
	h_buffhash=(uint8_t *)malloc(buffhash_size);
	cudaMalloc((void **)&d_buffhash, buffhash_size);
	...
	*correctwordnumber = -1;
    ...
    while(i < amountofwords) {
        ...
        generate_words(h_buff, word, wordsperrun);
		char hash_string[65];
		cudaMemcpy(d_buffhash, h_buffhash, buffhash_size, cudaMemcpyHostToDevice); 
		cudaMemcpy(d_buff, h_buff, buff_size, cudaMemcpyHostToDevice); 
		calc_sha_256<<<numberOfBlocks, threadsPerBlock>>>(d_buffhash, d_buff, wordsize, inthash, correctwordnumber);
		cudaMemcpy(h_buffhash, d_buffhash, buffhash_size, cudaMemcpyDeviceToHost);
		cudaMemcpy(h_buff, d_buff, buff_size, cudaMemcpyDeviceToHost);
	    cudaDeviceSynchronize();
        gettimeofday(&stop, NULL);
        i += wordsperrun;
		double h = i;
		double s = (double) ((stop.tv_sec - start0.tv_sec) * 1000000 + stop.tv_usec - start0.tv_usec) / 1000000; 
		printf("\rCurrentWord: %s\tHashrate: %f MH/s", h_buff, h/s/1000000);
		if (*correctwordnumber != -1) {
		 	uint8_t* tmphash = h_buffhash + (sizeof(uint8_t) * SIZE_OF_SHA_256_HASH) * *correctwordnumber;
		 	char* tmpstr = h_buff + (sizeof(char) * (wordsize + 1)) * *correctwordnumber;
